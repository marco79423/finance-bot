...
class LimitMarketData(MarketDataBase):
    def __init__(self, data, start_date=None, end_date=None):
        ...
        self._open = self.data.open.ffill()
        self._close = self.data.close.ffill()
        self._high = self.data.high.ffill()
        self._low = self.data.low.ffill()
        self._volume = self.data.volume.ffill()
