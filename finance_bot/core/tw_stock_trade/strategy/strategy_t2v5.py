...
class StrategyT2V5(StrategyBase):
    ...
    * 月營收三個月成長比率排名前 50 名
    * 根據月營收三個月成長比率排名順序買股
    * 大盤收盤價大於 100 日均線（牛市才買）
    * 成交量大於 100000 （成交量太少不買）
    賣股標準：
    * 大盤當日沒跌（大盤跌時不賣）
    * 若收盤價價低於月均線賣股或是持股超過 6 個月
    ...
    def handle(self):
        ...
        if self.data.close['0050'].iloc[-1] >= self.data.open['0050'].iloc[-1]:
            ...
            for stock_id in target_list:
                self.sell_next_day_market(stock_id, note=f'Out {self.growth_rate[stock_id] * 100:.2f}%')

            target_list = self.new_target_list([
                self.entry_date < self.today - pd.Timedelta(days=30 * self.params['long_num']),
            ], available_list=self.broker.holding_stock_ids)
            for stock_id in target_list:
                self.sell_next_day_market(stock_id, note=f'TooLong {self.growth_rate[stock_id] * 100:.2f}%')
